        ...
        for func, opt in zip(self.optimizer_func, self.optimizer):
            if optimizer == opt:
                if opt == "AdamW":
                    ...
                    with hp.conditional_scope("optimizer", [opt]):
                        model.compile(optimizer=func(learning_rate=LRSchedule()),
                                      loss="MSE")
                break
        return model


def get_ViT_tuner2(n_features, timelen):
...
class Encoder(Layer):
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        added1 = self.Add1([x, inputs])
        added2 = self.LN1(added1)
        x2 = self.MLP(added2)
        x3 = self.Add2([x2, added2])
        out = self.LN2(x3)
        return out
